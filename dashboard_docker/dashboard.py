...
from layouts.ia_zones import initialize_default_data, search_new_region, extract_industrial_zones, get_zones_dataframe , load_zones_from_db
...
#     if not region:
#         return {"status": "error", "message": "Aucune région fournie"}, 400

#     search_new_region(region)

#     return {"status": "ok", "message": f"Carte mise à jour pour {region}"}, 200

@app.server.route("/updateRegion", methods=["POST"])
def update_region():
    data = request.get_json()
    region = data.get("region")

    df = search_new_region(region)
...
@app.server.route("/getDefaultZones", methods=["GET"])
def get_default_zones():
    df = load_zones_from_db()
